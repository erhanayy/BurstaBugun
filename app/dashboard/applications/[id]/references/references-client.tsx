...
export default function ReferencesClient({ application, enableExemption, exemptionText }: { application: any, enableExemption?: boolean, exemptionText?: string }) {
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800 rounded-2xl p-6 md:p-8">
                ...
                {application.isExemptionRequested ? (
                    <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-900/40 rounded-xl p-8 text-center flex flex-col items-center justify-center">
                        <ShieldAlert className="w-12 h-12 text-blue-500 mx-auto mb-4" />
                        <h3 className="font-bold text-xl text-blue-900 dark:text-blue-100 mb-2">Muafiyet Talebiniz İncelemede</h3>
                        <p className="text-blue-700 dark:text-blue-300/80 text-base max-w-lg mx-auto">
                            Eski bursiyerlik beyanınız kurum yöneticilerinin onayına sunulmuştur. Onaylandıktan sonra referans aşamasını atlayarak doğrudan Bursiyer Havuzuna alınacaksınız.
                        </p>
                    </div>
                ) : (
                    <>
                        ...
                        {enableExemption && references.length === 0 ? (
                            <div className="bg-emerald-50 dark:bg-emerald-900/10 border border-emerald-200 dark:border-emerald-900/30 rounded-xl p-6 mb-8">
                                <h3 className="font-bold text-lg mb-3 text-emerald-900 dark:text-emerald-100">Özel Durum Bildirimi</h3>
                                <p className="text-sm text-emerald-800 dark:text-emerald-300/80 mb-6">
                                    Daha önce vakfımızdan burs aldıysanız, referans sürecinden muaf tutulmayı talep edebilirsiniz.
                                </p>
                                <div className="flex flex-col sm:flex-row gap-4">
                                    <Button 
                                        onClick={async () => {
                                            if(!confirm("Eski bursiyer beyanında bulunmak istediğinize emin misiniz?")) return;
                                            setIsSubmitting(true);
                                            try {
                                                const { requestExemption } = await import("@/lib/actions/reference");
                                                await requestExemption(application.id);
                                                toast.success("Muafiyet talebiniz yöneticiye iletildi.");
                                            } catch (err:any) {
                                    ...
                                    >
                                        Evet, {exemptionText || "Eski Bursiyeriyim"}
                                    </Button>
                                    <Button 
                                        variant="outline" 
                                        className="w-full sm:w-auto"
                                        onClick={() => {
                                            // Scroll down to the form or just let them see it below
                                            document.getElementById('reference-form-section')?.scrollIntoView({ behavior: 'smooth' });
                                        }}
                                    >
                                        Hayır, İlk Defa Başvuruyorum
                                    </Button>
                                </div>
                            </div>
                        ) : null}
                        ...
                        {!isFull ? (
                            <div id="reference-form-section" className="bg-blue-50 dark:bg-blue-900/10 border border-blue-100 dark:border-blue-900/30 rounded-xl p-6">
